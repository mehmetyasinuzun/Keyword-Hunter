...
func (s *Scraper) ScrapeURL(urlStr, title string) Content {
	...
	if err != nil {
		...
		logger.Error("Request creation error: %s - %v", urlStr, err)
		return content
	}
	...
	if err != nil {
		...
		logger.Warn("Connection error: %s - %s", urlStr, content.Error)
		return content
	}
	...
	if resp.StatusCode != 200 {
		...
		logger.Warn("HTTP error: %s - %d", urlStr, resp.StatusCode)
		return content
	}
...
func (s *Scraper) scrapeURLForExpand(urlStr string) Content {
	...
	if !strings.Contains(urlStr, ".onion") {
		...
		logger.Warn("Non-onion URL during expand: %s", urlStr)
		return content
	}
	...
	if err != nil {
		...
		logger.Error("Request creation error during expand: %s - %v", urlStr, err)
		return content
	}
	...
	if err != nil {
		// Hata sınıflandırması shared paketinden
		content.Error = shared.ClassifyError(err).Error()
		logger.Warn("Expand connection error: %s - %s", urlStr, content.Error)
		return content
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != 200 {
		...
		logger.Warn("Expand HTTP error: %s - %d", urlStr, resp.StatusCode)
		return content
	}
...
func (s *Scraper) CountKeywords(urlStr, keyword string) (int, error) {
	// Sayfayı çek
	content := s.scrapeURLForExpand(urlStr) // reuse for simplicity as it returns raw content
	if !content.Success {
