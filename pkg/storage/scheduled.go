...
// EnsureScheduledSchema scheduled_searches tablosunu oluşturur
func (db *DB) EnsureScheduledSchema() error {
	_, err := db.conn.Exec(`
		...
		CREATE INDEX IF NOT EXISTS idx_scheduled_due ON scheduled_searches(enabled, next_run_at);
	`)
	return err
}
