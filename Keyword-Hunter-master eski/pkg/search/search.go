...
// Arama motorlarının kendi domain'leri - bunları sonuçlardan çıkar
...
func New(torProxy string) (*Searcher, error) {
	...
	logger.Info("🔍 Searcher başlatıldı (timeout: %v)", shared.SearchTimeout)
...
func (s *Searcher) parseResults(html, sourceName string) []Result {
	...
	for _, match := range matches {
		if len(match) >= 3 {
			...
			// href içinde .onion var mı?
			onionURLs := onionRegex.FindAllString(href, -1)
			if len(onionURLs) > 0 {
				foundURL := cleanURL(onionURLs[0]) // URL'yi temizle
				...
				// Title çıkarma - HTML taglarını temizle
				title := htmlTagRegex.ReplaceAllString(innerHTML, "")
				...
				title = cleanTitle(title, foundURL)
...
func (s *Searcher) deduplicate(results []Result) []Result {
	...
	if filtered > 0 {
		logger.Debug("🔄 %d düşük değerli URL filtrelendi", filtered)
	}

	logger.Debug("✅ %d benzersiz sonuç", len(unique))
